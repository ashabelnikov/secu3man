...
void S3FImportController::OnOkPressed(void)
{
 ...
 if (mp_view->GetFWDFlag(FLAG_FTSCURVE_MAP))
  memcpy(mp_fwd->fts_curve, mp_s3f_io->GetData().fts_curve, sizeof(float) * (FTS_LOOKUP_TABLE_SIZE+2));

 if (mp_view->GetFWDFlag(FLAG_FUELDENSC_MAP))
  memcpy(mp_fwd->fueldens_corr, mp_s3f_io->GetData().fueldens_corr, sizeof(float) * FUELDENS_CORR_SIZE);

 if (mp_view->GetFWDFlag(FLAG_INJNONLINP_MAP))
  memcpy(mp_fwd->inj_nonlinp_corr, mp_s3f_io->GetData().inj_nonlinp_corr, sizeof(float) * INJ_NONLIN_SIZE * 2);
...
void S3FImportController::OnExchangePressed(void)
{
 ...
 if (mp_view->GetFWDFlag(FLAG_THRASS_MAP))
  memcpy(mp_fwd->maps[current_sel].inj_thrass, mp_s3f_io->GetData().maps[other_sel].inj_thrass,sizeof(float) * INJ_THRASS_SIZE);
...
void S3FImportController::OnViewActivate(void)
{
 ...
 mp_view->EnableFWDFlag(FLAG_FTSCURVE_MAP, sv0126 && sepmap); //since v01.26
...
void S3FExportController::OnOkPressed(void)
{
 ...
 if (mp_view->GetFWDFlag(FLAG_FTSCURVE_MAP))
  memcpy(mp_s3f_io->GetDataLeft().fts_curve, mp_fwd->fts_curve, sizeof(float) * (FTS_LOOKUP_TABLE_SIZE+2));

 if (mp_view->GetFWDFlag(FLAG_FUELDENSC_MAP))
  memcpy(mp_s3f_io->GetDataLeft().fueldens_corr, mp_fwd->fueldens_corr, sizeof(float) * FUELDENS_CORR_SIZE);

 if (mp_view->GetFWDFlag(FLAG_INJNONLINP_MAP))
  memcpy(mp_s3f_io->GetDataLeft().inj_nonlinp_corr, mp_fwd->inj_nonlinp_corr, sizeof(float) * INJ_NONLIN_SIZE * 2);
...
void S3FExportController::OnExchangePressed(void)
{
 ...
 if (mp_view->GetFWDFlag(FLAG_THRASS_MAP))
  memcpy(mp_s3f_io->GetDataLeft().maps[other_sel].inj_thrass, mp_fwd->maps[current_sel].inj_thrass, sizeof(float) * INJ_THRASS_SIZE);
