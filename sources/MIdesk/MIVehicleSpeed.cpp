...
{
 MeasInstrBase::Create(pParent, IDC_MI_VSS, false, false); //create window
...
void CMIVehicleSpeed::SetLimits(float loLimit, float upLimit)
{
 m_meter.SetRange(loLimit, upLimit, true); //<-- also update alert zones
}

void CMIVehicleSpeed::Append(const SECU3IO::SensorDat* i_values, bool i_revdir /*= false*/)
{
 MeasInstrBase::Append(i_values->speed, 0, 0, i_revdir);
}
...
void CMIVehicleSpeedGraph::Create(CWnd* pParent)
{
 ...
 MeasInstrBase::Create(pParent, IDC_MI_VSSGRAPH);

 // customize the control
 m_scope.SetRange(0.0, 220);
...
void CMIVehicleSpeedGraph::Append(const SECU3IO::SensorDat* i_values, bool i_revdir /*= false*/)
{
 MeasInstrBase::Append(i_values->speed, i_revdir);
}
