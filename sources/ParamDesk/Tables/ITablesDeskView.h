...
class ITablesDeskView
{
 public:
  ...
  virtual void SetDynamicValues(int rpm, float temp, int air_flow, float adv_ang, float knock_retard, bool knkret_use,//set values dynamycally indicated in the GMW window
   ...
   float airt_aalt, bool airt_use, float idlreg_aac, bool idlreg_use, float octan_aac, bool octan_use, float tps, float iac_pos, int tpsdot, float voltage, float add_i1, float tmp2, float baro_press, float load, float afr, bool acceleration) = 0;
