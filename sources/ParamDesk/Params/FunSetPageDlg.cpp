...
//TODO: ≈сли будет необходимость использовать режим сортировки, то дл€ корректного
//ассоциировани€ элементов можно использовать CComboBox::SetItemData. ¬ любом случае
//должна изменитьс€ только внутренн€€ архитектура этого диалога

//заполн€ет ComboBox-ы имен семейств характеристик
void CFunSetPageDlg::FillCBByFunNames(void)
{
 if (!::IsWindow(m_hWnd))
  return;

 //удал€ем предыдущий контент
 m_benzin_maps_combo.ResetContent();
 m_gas_maps_combo.ResetContent();

 //добавл€ем новый контент
 ...
 //после удалени€ всех элементов текущее выделение в ComboBox-ах
 //тер€етс€ и мы должны вернуть старое выделение.
