...
bool CCommunicationManager::Init(void)
{
  ...
  //оповещаем объекта слушател€ об изменении настроек
...
void CCommunicationManager::SwitchOn(size_t i_cntr, bool i_force_reinit  /* = false */)
{
 switch(i_cntr)
 {
  case OP_ACTIVATE_APPLICATION:
   ...
   {
   //все контроллеры в активном состо€нии?
...
void CCommunicationManager::OnStartLogWriting(void)
{
  ...
  {
   CString  string;
   string.Format(_T("Ќе могу начать запись лога в файл: %s\n\
¬озможно этот файл защищен от записи или уже открыт другой программой."), full_file_name.c_str());
