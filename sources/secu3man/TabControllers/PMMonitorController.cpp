   ...
   if (!m_show_raw_sensors)
   {
    ...
    else
    {
     ...
     mp_tdview->SetDynamicValues(sd->frequen, sd->temperat, sd->air_flow, sd->adv_angle, sd->knock_retard, sd->knkret_use, sd->strt_aalt, sd->strt_use,
      ...
      sd->idlreg_aac, sd->idlreg_use, sd->octan_aac, sd->octan_use, sd->tps, sd->choke_pos, sd->tpsdot, sd->voltage, sd->add_i1, sd->tmp2, sd->baro_press, sd->load, sd->afr, sd->acceleration, sd->ephh_valve);
