...
void CFirmwareTabController::OnEditFwConsts(void)
{
 ...
 if (mp_settings->GetInterfaceLanguage() == IL_RUSSIAN)
  dfd.AppendItem(_T("Время стационарности рабочей точки в тактах"), _T("т"), .0f, 255.0f, 1.0f, 0, &d.ltft_stab_str, _T("Адаптация ячейки происходит только когда рабочая точка находится в ее зоне в течение этого числа тактов. Если это значение установлено в 0, то вместо тактов используется время."));
 else
