...
, mp_moncntr(new CPMMonitorController(ip_view->mp_MIDeskDlg.get(),ip_view->mp_RSDeskDlg.get(), ip_view->mp_CEDeskDlg.get(), ip_view->mp_TablesDeskDlg.get(), ip_view->mp_SeptabsDeskDlg.get(), ip_comm, ip_sbar, ip_settings))
...
void CParamMonTabController::OnPacketReceived(const BYTE i_descriptor, SECU3IO::SECU3Packet* ip_packet)
{
 ...
 if (m_current_state != m_state_machine.end() && (*m_current_state)->CollectData(i_descriptor, ip_packet))
 {
  ...
  //Завершилось чтение отдельных таблиц
...
void CParamMonTabController::OnParametersChanged(const BYTE i_descriptor, const void* ip_packet)
{
 if (i_descriptor == FUNSET_PAR)
 {
  ...
  mp_sepcntr->OnFunSetChanged((const FunSetPar*)ip_packet);
 }
}
...
void CParamMonTabController::OnMIDeskSettingsChanged(void)
{
 ...
 IndCoolingFan, IndCE, IndFCRevLim, IndFloodClear, IndSysLocked, IndIgn_i, IndCond_i, IndEpas_i, IndAftStrEnr, IndIacClLoop, IndUniOut1, IndUniOut2, IndUniOut3, IndUniOut4, IndUniOut5, IndUniOut6, IndGpa4_i, IndInput1, IndInput2, IndAuto_i, IndMapsel0, IndRefprs_i, IndAltrn_i;

 mp_view->mp_MIDeskDlg->GetIndicatorsCfg(IndHeightPercent, IndRows, IndGas_v, IndCarb, IndIdleValve, IndPowerValve, IndStBlock, IndAE, IndCoolingFan, IndCE, IndFCRevLim, IndFloodClear, IndSysLocked, IndIgn_i, IndCond_i, IndEpas_i, IndAftStrEnr, IndIacClLoop, IndUniOut1, IndUniOut2, IndUniOut3, IndUniOut4, IndUniOut5, IndUniOut6, IndGpa4_i, IndInput1, IndInput2, IndAuto_i, IndMapsel0, IndRefprs_i, IndAltrn_i);

 cfg0.m_optIndHeightPercent[idx] = IndHeightPercent, cfg0.m_optIndRows[idx] = IndRows, cfg0.m_optIndGas_v[idx] = IndGas_v, cfg0.m_optIndCarb[idx] = IndCarb, cfg0.m_optIndIdleValve[idx] = IndIdleValve, cfg0.m_optIndPowerValve[idx] = IndPowerValve, cfg0.m_optIndStBlock[idx] = IndStBlock, cfg0.m_optIndAE[idx] = IndAE, cfg0.m_optIndCoolingFan[idx] = IndCoolingFan, cfg0.m_optIndCE[idx] = IndCE, cfg0.m_optIndFCRevLim[idx] = IndFCRevLim, cfg0.m_optIndFloodClear[idx] = IndFloodClear, cfg0.m_optIndSysLocked[idx] = IndSysLocked, cfg0.m_optIndIgn_i[idx] = IndIgn_i, cfg0.m_optIndCond_i[idx] = IndCond_i, cfg0.m_optIndEpas_i[idx] = IndEpas_i, cfg0.m_optIndAftStrEnr[idx] = IndAftStrEnr, cfg0.m_optIndIacClLoop[idx] = IndIacClLoop, cfg0.m_optIndUniOut1[idx] = IndUniOut1, cfg0.m_optIndUniOut2[idx] = IndUniOut2, cfg0.m_optIndUniOut3[idx] = IndUniOut3, cfg0.m_optIndUniOut4[idx] = IndUniOut4, cfg0.m_optIndUniOut5[idx] = IndUniOut5, cfg0.m_optIndUniOut6[idx] = IndUniOut6, cfg0.m_optIndGpa4_i[idx] = IndGpa4_i, cfg0.m_optIndInput1[idx] = IndInput1, cfg0.m_optIndInput2[idx] = IndInput2, cfg0.m_optIndAuto_i[idx] = IndAuto_i, cfg0.m_optIndMapsel0[idx] = IndMapsel0, cfg0.m_optIndRefprs_i[idx] = IndRefprs_i, cfg0.m_optIndAltrn_i[idx] = IndAltrn_i;

 for (int ii = 0; ii < 2; ++ii) //copy colors, redundant because we have single color set in ini file
  cfg0.m_optIndGas_v[ii].second = IndGas_v.second, cfg0.m_optIndCarb[ii].second = IndCarb.second, cfg0.m_optIndIdleValve[ii].second = IndIdleValve.second, cfg0.m_optIndPowerValve[ii].second = IndPowerValve.second, cfg0.m_optIndStBlock[ii].second = IndStBlock.second, cfg0.m_optIndAE[ii].second = IndAE.second, cfg0.m_optIndCoolingFan[ii].second = IndCoolingFan.second, cfg0.m_optIndCE[ii].second = IndCE.second, cfg0.m_optIndFCRevLim[ii].second = IndFCRevLim.second, cfg0.m_optIndFloodClear[ii].second = IndFloodClear.second, cfg0.m_optIndSysLocked[ii].second = IndSysLocked.second, cfg0.m_optIndIgn_i[ii].second = IndIgn_i.second, cfg0.m_optIndCond_i[ii].second = IndCond_i.second, cfg0.m_optIndEpas_i[ii].second = IndEpas_i.second, cfg0.m_optIndAftStrEnr[ii].second = IndAftStrEnr.second, cfg0.m_optIndIacClLoop[ii].second = IndIacClLoop.second, cfg0.m_optIndUniOut1[ii].second = IndUniOut1.second, cfg0.m_optIndUniOut2[ii].second = IndUniOut2.second, cfg0.m_optIndUniOut3[ii].second = IndUniOut3.second, cfg0.m_optIndUniOut4[ii].second = IndUniOut4.second, cfg0.m_optIndUniOut5[ii].second = IndUniOut5.second, cfg0.m_optIndUniOut6[ii].second = IndUniOut6.second, cfg0.m_optIndGpa4_i[ii].second = IndGpa4_i.second, cfg0.m_optIndInput1[ii].second = IndInput1.second, cfg0.m_optIndInput2[ii].second = IndInput2.second, cfg0.m_optIndAuto_i[ii].second = IndAuto_i.second, cfg0.m_optIndMapsel0[ii].second = IndMapsel0.second, cfg0.m_optIndRefprs_i[ii].second = IndRefprs_i.second, cfg0.m_optIndAltrn_i[ii].second = IndAltrn_i.second;
