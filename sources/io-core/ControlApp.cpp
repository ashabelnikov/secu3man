...
bool CControlApp::Parse_FUNSET_PAR(const BYTE* raw_packet, size_t size)
{
 ...
 //APPS1 curve offset
 int apps1_curve_offset = 0;
 if (false == mp_pdp->Hex16ToBin(raw_packet, &apps1_curve_offset, true))
  return false;
 funSetPar.apps1_curve_offset = ((float)apps1_curve_offset) * m_adc_discrete;

 //APPS1 curve gradient
 int apps1_curve_gradient = 0;
 if (false == mp_pdp->Hex16ToBin(raw_packet, &apps1_curve_gradient, true))
  return false;
 funSetPar.apps1_curve_gradient = ((float)apps1_curve_gradient) / ((APPS_MULT*2) * m_adc_discrete * 128.0f);

 //accelerator pedal position sensor
 int apps1_raw = 0;
 if (false == mp_pdp->Hex16ToBin(raw_packet, &apps1_raw))
  return false;
 funSetPar.apps1_raw = apps1_raw * m_adc_discrete;
...
bool CControlApp::Parse_EDITAB_PAR(const BYTE* raw_packet, size_t size)
{
 ...
 if (!(editTabPar.tab_id >= ETMT_SET_START && editTabPar.tab_id <= ETMT_SET_END) && 
     !(editTabPar.tab_id >= ETMT_SEP_START && editTabPar.tab_id <= ETMT_SEP_END) && editTabPar.tab_id != ETMT_NAME_STR)
  return false;
 ...
 if (editTabPar.tab_id != ETMT_NAME_STR)
 {
  ...
  else
  {
   ...
   else if (editTabPar.tab_id == ETMT_INJ_IT) //injection timing map
   {
    BYTE buff[32]; unsigned char value;
    for(size_t i = 0; i < size; ++i)
    {     
...
bool CControlApp::Parse_INJCTR_PAR(const BYTE* raw_packet, size_t size)
{
 ...
 if (size != 51)  //размер пакета без сигнального символа, дескриптора и символа-конца пакета
  return false;
 ...
 //Injection PW ROC - decrease limit
 int  injpw_dec_speed;
 if (false == mp_pdp->Hex16ToBin(raw_packet, &injpw_dec_speed, true))
  return false;
 injctrPar.injpw_dec_speed = float(injpw_dec_speed) * (3.2f / 1000.0f);
 //Injection PW ROC - increase limit
 int  injpw_inc_speed;
 if (false == mp_pdp->Hex16ToBin(raw_packet, &injpw_inc_speed, true))
  return false;
 injctrPar.injpw_inc_speed = float(injpw_inc_speed) * (3.2f / 1000.0f);
...
void CControlApp::Build_FUNSET_PAR(FunSetPar* packet_data)
{
 ...
 int apps1_curve_offset = MathHelpers::Round(packet_data->apps1_curve_offset / m_adc_discrete);
 mp_pdp->Bin16ToHex(apps1_curve_offset, m_outgoing_packet);
 int apps1_curve_gradient = MathHelpers::Round(128.0f * packet_data->apps1_curve_gradient * (APPS_MULT*2) * m_adc_discrete);
 mp_pdp->Bin16ToHex(apps1_curve_gradient, m_outgoing_packet);
}
...
void CControlApp::Build_EDITAB_PAR(EditTabPar* packet_data)
{
 ...
 else if (packet_data->tab_id == ETMT_INJ_IT) //injection timing map
 {
  BYTE buff[64];  
  int byte_idx_start = packet_data->address & 0x1;
...
/*TRACE("\n");
 ...
 else if (packet_data->tab_id != ETMT_NAME_STR)
 {
  ...
  for(unsigned int i = 0; i < packet_data->data_size; ++i)
  {
   if (packet_data->tab_id == ETMT_INJ_CRNK || packet_data->tab_id == ETMT_INJ_DEAD || packet_data->tab_id == ETMT_DWELLCNTRL)
   {
    int value = MathHelpers::Round((packet_data->table_data[i] * 1000.0f) / discrete);
    mp_pdp->Bin16ToHex(value, m_outgoing_packet);
   }
   else if (packet_data->tab_id == ETMT_INJ_AFR)
   {
   ...
   else if (packet_data->tab_id == ETMT_PWM1)
   {
   ...
   else if (packet_data->tab_id == ETMT_INJ_AERPM)
   {
   ...
   else if (packet_data->tab_id == ETMT_INJ_IACC)
   {
   ...
   else if (packet_data->tab_id == ETMT_INJ_IACCW)
   {
    int value;
   ...
   else if (packet_data->tab_id == ETMT_INJ_IATCLT)
   {
   ...
   else if (packet_data->tab_id == ETMT_INJ_ATSC)
   {
    unsigned char value = MathHelpers::Round(packet_data->table_data[i] * 128.0f);
    mp_pdp->Bin8ToHex(value, m_outgoing_packet);
   }
   else if (packet_data->tab_id == ETMT_INJ_IACMAT)
   {
    signed char value = MathHelpers::Round(packet_data->table_data[i] * 4.0f);
    mp_pdp->Bin8ToHex(value, m_outgoing_packet);
   }
   else if (packet_data->tab_id == ETMT_INJ_CYLMULT)
   {
    unsigned char value = MathHelpers::Round((packet_data->table_data[i] - 0.5f) * 256.0f);
    mp_pdp->Bin8ToHex(value, m_outgoing_packet);
   }
   else if (packet_data->tab_id == ETMT_INJ_CYLADD)
   {
    signed char value = MathHelpers::Round(packet_data->table_data[i] / 0.0256f);
    mp_pdp->Bin8ToHex(value, m_outgoing_packet);
   }
   else if (packet_data->tab_id == ETMT_INJ_AEMAP)
   {
...
void CControlApp::Build_INJCTR_PAR(InjctrPar* packet_data)
{
 ...
 int injpw_dec_speed = MathHelpers::Round(packet_data->injpw_dec_speed * (1000.0f / 3.2f));
 mp_pdp->Bin16ToHex(injpw_dec_speed, m_outgoing_packet);
 int injpw_inc_speed = MathHelpers::Round(packet_data->injpw_inc_speed * (1000.0f / 3.2f));
 mp_pdp->Bin16ToHex(injpw_inc_speed, m_outgoing_packet);
}
