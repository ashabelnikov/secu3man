...
//данные
const char cCSVDataTemplateString[] = "%c%%d%c%%f%c%%f%c%%f%c%%f%c%%f%c%%f%c%%d%c%%d%c%%d%c%%d%c%%d%c%%d%c%%d%c%%d%c%%f%c%%f%c%%f%c%%s\r\n";
...
bool LogReader::GetRecord(SYSTEMTIME& o_time, SECU3IO::SensorDat& o_data)
{
 ...
 float pressure,voltage,temperat,adv_angle,knock_k, knock_retard, tps, add_i1, add_i2;
 char ce_errors[20] = {0};
...
void LogReader::SetSeparatingSymbol(char i_sep_symbol)
{
 ...
 sprintf (m_csv_data_template, cCSVDataTemplateString, x, x, x, x, x, x, x, x, x, x, x, x, x, x, x, x, x, x, x);
}
