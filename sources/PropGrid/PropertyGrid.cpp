...
HITEM CPropertyGrid::AddItem(HSECTION hs, EItemType type, _TSTRING name, void* pValue, bool editable, bool undefined, HITEM after)
{
  ...
  else if (type == IT_STRING || type == IT_TEXT || type == IT_FILE || type == IT_FOLDER) item.m_strValue = *(_TSTRING*)pValue;
  else if (type == IT_COMBO || type == IT_INTEGER || type == IT_HEX) item.m_nValue = *(int*)pValue;
...
HITEM CPropertyGrid::AddCustomItem(HSECTION section, _TSTRING name, ICustomItem* pItem, bool editable, HITEM after)
{
  pItem->m_pGrid = this;
...
bool CPropertyGrid::GetItemValue(HITEM item, int& nValue) const
{
  ...
  // check
  if (pItem->m_type == IT_COMBO || pItem->m_type == IT_INTEGER || pItem->m_type == IT_HEX)
  {
