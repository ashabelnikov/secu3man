...
// 18 December 2009 by Alexey Shabelnikov:
//    - Removed costom controls
//    - Added "selector" control
...
void CPropertyGrid::SetTrueFalseStrings(_TSTRING strTrue, _TSTRING strFalse)
{
 m_strTrue = strTrue;
 m_strFalse = strFalse;
}

void CPropertyGrid::SetOkCancelStrings(_TSTRING strOk, _TSTRING strCancel)
{
 m_strOk = strOk;
 m_strCancel = strCancel;
}

void CPropertyGrid::SetDateTimeStrings(_TSTRING strDate, _TSTRING strTime)
{
 m_strDate = strDate;
 m_strTime = strTime;
}

void CPropertyGrid::SetUndefinedString(_TSTRING strUndefined)
{
 m_strUndefined = strUndefined;
}

void CPropertyGrid::SetEmptyString(_TSTRING strEmpty)
{
 m_strEmpty = strEmpty;
}
...
bool CPropertyGrid::CItem::operator==(const _TSTRING& name) const
{
 return m_name == name;
}
...
HSECTION CPropertyGrid::AddSection(_TSTRING title, bool collapsed, HSECTION after)
{
 // build it
 CSection section;
...
HITEM CPropertyGrid::AddItem(HSECTION hs, EItemType type, _TSTRING name, void* pValue, bool editable, bool undefined, HITEM after)
{
...
HITEM CPropertyGrid::AddStringItem(HSECTION section, _TSTRING name, _TSTRING value, bool editable, HITEM after)
{
 return AddItem(section, IT_STRING, name, &value, editable, false, after);
}

HITEM CPropertyGrid::AddTextItem(HSECTION section, _TSTRING name, _TSTRING value, bool editable, HITEM after)
{
 return AddItem(section, IT_TEXT, name, &value, editable, false, after);
}

HITEM CPropertyGrid::AddIntegerItem(HSECTION section, _TSTRING name, int value, _TSTRING format, bool editable, bool undefined, HITEM after, const CPropertyGridInPlaceEdit::InplaceEditParamsEx* ip_ex_params /*= NULL*/)
{
 ...
 if (pItem) 
 {
  ...
  if (NULL!=ip_ex_params)
   pItem->m_SpinEdExParams = (*ip_ex_params);
 }
 return it;
}

HITEM CPropertyGrid::AddHexItem(HSECTION section, _TSTRING name, int value, _TSTRING format, bool editable, bool undefined, HITEM after, const CPropertyGridInPlaceEdit::InplaceEditParamsEx* ip_ex_params /*= NULL*/)
{
...
HITEM CPropertyGrid::AddDoubleItem(HSECTION section, _TSTRING name, double value, _TSTRING format, bool editable, bool undefined, HITEM after, const CPropertyGridInPlaceEdit::InplaceEditParamsEx* ip_ex_params /*= NULL*/)
{
...
HITEM CPropertyGrid::AddComboItem(HSECTION section, _TSTRING name, const vector<_TSTRING>& values, int cur, bool editable, bool undefined, HITEM after)
{
...
HITEM CPropertyGrid::AddBoolItem(HSECTION section, _TSTRING name, bool value, bool editable, bool undefined, HITEM after)
{
 return AddItem(section, IT_BOOLEAN, name, &value, editable, undefined, after);
}

HITEM CPropertyGrid::AddDateItem(HSECTION section, _TSTRING name, COleDateTime value, _TSTRING format, bool editable, bool undefined, HITEM after)
{
...
HITEM CPropertyGrid::AddDateTimeItem(HSECTION section, _TSTRING name, COleDateTime value, _TSTRING format, bool editable, bool undefined, HITEM after)
{
...
HITEM CPropertyGrid::AddFileItem(HSECTION section, _TSTRING name, _TSTRING value, _TSTRING filter, bool editable, HITEM after)
{
...
HITEM CPropertyGrid::AddFolderItem(HSECTION section, _TSTRING name, _TSTRING value, _TSTRING title, bool editable, HITEM after)
{
...
HITEM CPropertyGrid::AddColorItem(HSECTION section, _TSTRING name, COLORREF value, bool editable, bool undefined, HITEM after)
{
 return AddItem(section, IT_COLOR, name, &value, editable, undefined, after);
}

HITEM CPropertyGrid::AddFontItem(HSECTION section, _TSTRING name, LOGFONT value, bool editable, bool undefined, HITEM after)
{
 return AddItem(section, IT_FONT, name, &value, editable, undefined, after);
}

HITEM CPropertyGrid::AddSelectorItem(HSECTION section, _TSTRING name, const std::vector<_TSTRING>& values, int cur, _TSTRING unit /*=_T("")*/, bool editable /*= true*/, bool undefined /*= false*/, HITEM after /*= -1*/)
{
...
bool CPropertyGrid::GetItemValue(HITEM item, _TSTRING& strValue) const
{
 // get the item
...
bool CPropertyGrid::GetItemValue(HITEM item, int& nValue) const
{
 ...
 // check
 if (pItem->m_type == IT_COMBO || pItem->m_type == IT_INTEGER || pItem->m_type == IT_HEX || pItem->m_type == IT_SELECTOR)
 {
...
bool CPropertyGrid::SetItemValue(HITEM item, const _TSTRING strValue)
{
 // get the item
 CItem* pItem = FindItem(item);
 if (pItem == NULL) return false;
...
BEGIN_MESSAGE_MAP(CPropertyGrid, CWnd)
 ...
 ON_MESSAGE(WM_PG_SELECTORSELCHANGED, OnComboSelChanged)
...
void CPropertyGrid::DrawItem(CDC& dc, int w, int x, int y, vector<CItem>::iterator& it)
{
 ...
 // is being edited?
 if (m_focused_item == it->m_id && it->m_editable)
 {
 ...
 // now draw text
 _TSTRING strValue = it->m_strValue;
 switch (it->m_type)
 {
  case IT_TEXT:
   {
  ...
  case IT_SELECTOR:
   {
    if (it->m_nValue>=0 && it->m_nValue<int(it->m_options.size()))
    {
     ...
     strValue+= it->m_unit; //append unit string
    }
    modified = (it->m_nValue != it->m_nValue_old);
    break;
   }
...
{
 if (m_button_pushed || m_value_clicked)
 {
  m_button_pushed = false;
  m_button_depressed = false;
  ReleaseCapture();
...
void CPropertyGrid::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
 ...
 else if (nChar == VK_RIGHT)
 {
  ...
  _ActivateFocusedItem(); //activate editing by space bar key
...
LRESULT CPropertyGrid::OnComboSelChanged(WPARAM wParam, LPARAM lParam)
{
 ...
 if (pItem)
 {
  ...
  else if (pItem->m_type == IT_COMBO || pItem->m_type == IT_SELECTOR)
  {
...
void CPropertyGrid::_ActivateFocusedItem(void)
{
 CItem* pItem = FindItem(m_focused_item);
 if (pItem)
 {
  if (GetEditMode(*pItem) == EM_DROPDOWN)
  {
   ...
   else
   {
    ...
    {
     for (vector<_TSTRING>::iterator it = pItem->m_options.begin(); it != pItem->m_options.end(); ++it)
      pCombo->AddString(*it);
     if (!pItem->m_undefined)
  ...
  else if (GetEditMode(*pItem) == EM_INPLACE)
  {
   ...
   {//selector!      
    ...
    pSelector->SetColors(m_clrBack, m_clrText);
    pSelector->SetFont(&m_fntNormal);
